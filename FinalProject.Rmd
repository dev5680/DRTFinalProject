...
leaflet(business_resturant_NVCoords) %>% addProviderTiles("Esri.NatGeoWorldMap") %>%
  addCircles(lng = ~longitude, lat = ~latitude,radius = ~sqrt(stars>=4))  %>%
...
```{r}
business_resturant_NVCoords = business_resturant_NV_AZ %>% filter(state == "AZ")
